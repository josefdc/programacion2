...
void primerosNFibonacci(int n, int current) {
    if (current <= n) {
        printf("%d ", fibonacciN(current));
        primerosNFibonacci(n, current + 1);
    } else {
        printf("\n");
    }
}
...
void torresDeHanoi(int n, char origen, char aux, char destino) {
    if (n == 1) {
        printf("Mover anillo 1 desde torre %c a torre %c\n", origen, destino);
    } else {
        torresDeHanoi(n - 1, origen, destino, aux);
        printf("Mover anillo %d desde torre %c a torre %c\n", n, origen, destino);
        torresDeHanoi(n - 1, aux, origen, destino);
    }
}

            printf("Introduce el numero de anillos para resolver las torres de Hanoi: ");
            scanf("%d", &n);
        ...
        case 17:
            ...
            break;
        default:
            printf("Opcion no valida. Por favor, seleccione una opcion entre 1 y 17.\n");
            break;
    }
    mostrarMenu();  // Display the menu again after the operation is completed
}
